...
## 🆘 Windows でダウンロードや起動がブロックされる

ダウンロード時に **「一般的にダウンロードされていません」** や、
起動時に **「Windows によって PC が保護されました」** という警告が出ることがあります。

これは Windows の SmartScreen 機能が「ダウンロード数の少ない exe ファイル」を
自動的に警告するためで、**ファイルに問題があるわけではありません**
（ただ単に、まだあまり使われていないアプリだから）。

### ダウンロード時にブロックされたら

1. ブラウザの「ダウンロード」一覧で当該ファイルの行の **「…」(三点)** メニューを開く
2. **「保持する」** （英語版は **"Keep"**）を選ぶ
3. 確認ダイアログ → **「危険なファイルを保持する」** / **"Keep anyway"** を選択

### 起動時にブロックされたら

「Windows によって PC が保護されました」のブルー画面で:

1. 画面下の小さい **「詳細情報」** リンクをクリック
2. その下に出てくる **「実行」** ボタンを押す

### PowerShell から直接ダウンロード（警告なし）

```powershell
Invoke-WebRequest `
  -Uri "https://github.com/mar1mo-41414/MineScale/releases/latest/download/mc-share-gui-windows-x64.exe" `
  -OutFile "mc-share-gui.exe"
```
...
| 公式アカウント (MSA) | ✅ |
| オフラインアカウント | ⚠️ ホスト側の online-mode 設定次第（「LAN 公開」なら基本 OK、PaperMC は online-mode=true がデフォルト） |

### 長時間セッション

- Sym NAT ジョイナー経由の relay 経路で **連続 3 時間+ セッション** 実測稼働（v1.2.13 で確認）
- 15 分の Room 期限切れが既存接続に影響しないことも実証済み
- サーバー側のメモリリークなし（RSS 変動 ±0.5 MB、FD 数安定）
...
- v1.2.12〜v1.2.13: **長時間セッションの安定性を修正**。15 分の Room 期限切れが既存接続を切断するバグ、および Room 期限切れ後の relay 再接続爆撃を停止。3 時間以上の連続プレイで問題なく稼働することを実測確認
...
| `outcome` | `success` / `cancelled` / `stun_failed` / `punch_failed` / `tls_failed` / ... | 成否カテゴリ（`result` のみ） |
| `transport` | `quic` / `relay` | 実際に使われた経路（接続成立時のみ） |
